...
class GameBoard:
    ...
    # _npcs:
    #   dictionary with keys of character subclasses that are not the player
    #   (raccoon, RecyclingBin, and  GarbageCan), and
    #   values of list of all of that type of character
    ended: bool
    turns: int
    width: int
    height: int
    _player: Optional[Player]
    _npcs: Dict[str, List[Character]]
    ...
    def place_character(self, c: Character) -> None:
        ...
        if isinstance(c, Player):
            ...
            self._npcs["raccoons"].append(c)
        elif isinstance(c, RecyclingBin):
            self._npcs["recycling_bins"].append(c)
        elif isinstance(c, GarbageCan):
            self._npcs["garbage_cans"].append(c)
    ...
    def at(self, x: int, y: int) -> List[Character]:
        ...
        if not self.on_board(x, y):
            return []
        if self._player:
            if (self._player.x, self._player.y) == target:
                results.append(self._player)
        for key in self._npcs:
            for npc in self._npcs[key]:
        ...
        for j in range(self.height):
            ...
            for i in range(self.width):
                ...
                if len(chars) == 0:
                    ...
                    row_result.append(chars[0].get_char())
                elif len(chars) == 2:
                    # Because the only time the length would be two would be
                    # when a raccoon is inside a trash can
                    row_result.append("@")
    ...
    def give_turns(self) -> None:
        ...
        if self.turns % RACCOON_TURN_FREQUENCY == 0:  # PROVIDED, DO NOT CHANGE
            for raccoon in self._npcs["raccoons"]:
    ...
    def adjacent_bin_score(self) -> int:
        ...
        """
        max_score = 0
        for r_bin in self._npcs["recycling_bins"]:
            score = self._check_cluster_score(r_bin)
            if score > max_score:
                max_score = score
        return max_score

    def _check_cluster_score(self, r_bin: RecyclingBin) -> int:
        """Return the size of the cluster originating from a
        specific bin (r_bin) on this board
        >>> b = GameBoard(4, 4)
        >>> r1 = RecyclingBin(b, 1, 1)
        >>> r2 = RecyclingBin(b, 1, 2)
        >>> r3 = RecyclingBin(b, 3, 3)
        >>> b._check_cluster_score(r1)
        2
        >>> b._check_cluster_score(r2)
        2
        >>> b._check_cluster_score(r3)
        1
        """
        score = 1
        already_checked = {(r_bin.x, r_bin.y)}
        to_check_tiles = get_neighbours((r_bin.x, r_bin.y))
        while len(to_check_tiles) != 0:
            current_check = to_check_tiles.copy()
            for tile in current_check:
                tile_chars = self.at(tile[0], tile[1])
                if tile in already_checked:
                    pass
                elif len(tile_chars) == 1 and \
                        isinstance(tile_chars[0], RecyclingBin):
                    score += 1
                    new_neighbors = get_neighbours((tile[0], tile[1]))
                    to_check_tiles.extend(new_neighbors)
                already_checked.add(tile)
                to_check_tiles.remove(tile)
        return score
...
class RecyclingBin(Character):
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        if not self.board.on_board(target_x, target_y):
            return False
        target_characters = self.board.at(target_x, target_y)
        if len(target_characters) == 0:
            self.x = target_x
            self.y = target_y
            return True
        if len(target_characters) == 1:
            if isinstance(target_characters[0], RecyclingBin):
                if target_characters[0].move(direction):
                    self.x = target_x
                    self.y = target_y
                    return True
        return False
...
class Raccoon(TurnTaker):
    ...
    def check_trapped(self) -> bool:
        ...
        for direction in DIRECTIONS:
            target_x = self.x + direction[0]
            target_y = self.y + direction[1]
            if self.board.on_board(target_x, target_y):
                target_chars = self.board.at(target_x, target_y)
                if len(target_chars) == 0:
                    return False
                elif len(target_chars) == 1 and \
                        target_chars[0].get_char() == "O":
                    return False
        return True
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        if self.inside_can:
            return False
        target_x = direction[0] + self.x
        target_y = direction[1] + self.y
        if self.board.on_board(target_x, target_y):
            target_chars = self.board.at(target_x, target_y)
            if len(target_chars) == 0:
                self.x = target_x
                ...
                return True
                    ...
                    isinstance(target_chars[0], GarbageCan):
                garbage_can = target_chars[0]
                if garbage_can.get_char() == "O":
                    self.x = target_x
                    self.y = target_y
                    self.inside_can = True
                    return True
                elif garbage_can.get_char() == "C":
    ...
    def take_turn(self) -> None:
        ...
        (2, 1)
        """
        if self.inside_can or self.check_trapped():
            return None
        moved = False
        while not moved:
            direction = random.randint(0, 3)
            moved = self.move(DIRECTIONS[direction])
...
class SmartRaccoon(Raccoon):
    ...
    def take_turn(self) -> None:
        ...
        True
        """
        spots_to_check = get_neighbours((self.x, self.y))
        directions_to_check = DIRECTIONS.copy()
        index = 0
        found_can = False
        while not found_can:
            if len(spots_to_check) == 0:
                # If there are no garbage cans in any directions do normal move
                Raccoon.take_turn(self)
                break
            index = index % len(spots_to_check)
            spot_x = spots_to_check[index][0]
            spot_y = spots_to_check[index][1]
            if not self.board.on_board(spot_x, spot_y):
                spots_to_check.pop(index)
                directions_to_check.pop(index)
                continue
            target_chars = self.board.at(spot_x, spot_y)
            if len(target_chars) == 1 and \
                    isinstance(target_chars[0], GarbageCan):
                # if it's a garbage can move towards the spot
                found_can = True
                self.move(directions_to_check[index])
            elif len(target_chars) == 0 or target_chars[0].get_char() == "P":
                # if it's a player or empty space add the next furthest spot to
                # check on the next iteration
                dir_x = directions_to_check[index][0]
                dir_y = directions_to_check[index][1]
                spots_to_check[index] = (spot_x + dir_x, spot_y + dir_y)
                index += 1
            else:
                # if it's anything else stop looking in that direction
                spots_to_check.pop(index)
                directions_to_check.pop(index)
